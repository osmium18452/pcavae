...
class ILSTMCVAE:
    def __init__(self, dataloader, latent_size, gpu, learning_rate, gpu_device):
        ...
        for i, size in enumerate(self.cvae_dim_list):
            self.cvae_list.append(LSTMCVAE(size, latent_size, i))
            if gpu:
...
if __name__ == '__main__':
    ...
    ilstmcvae = ILSTMCVAE(dataloader, 5, True, .01, '1,2,3')
    ilstmcvae.train_vaes_in_serial(epoch,batch_size,gpu=True)
